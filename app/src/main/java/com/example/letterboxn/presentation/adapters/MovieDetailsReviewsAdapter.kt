...
//            ReviewratingBar.rating = reviewItem.reviewRating
//
...
//    }
//
//    fun updateAdapter(newReviews : List<ReviewItem>) {
//        reviews = newReviews
...
class MovieDetailsReviewsAdapter(
    val bindinqa : FragmentDetailsMovieBinding,
    val reviews : List<ReviewWithoutMovieItem>,
    val onClick: (ReviewWithoutMovieItem) -> Unit
) : BaseAdapter<SampleReviewMoviedetailsBinding>(SampleReviewMoviedetailsBinding::inflate) {

    override fun getItemCount(): Int {
        if (reviews.isEmpty()) bindinqa.textNoReviewsSentenceMovieDetails.visibility = View.VISIBLE
        else bindinqa.textNoReviewsSentenceMovieDetails.visibility = View.GONE
        if (reviews.size > 10) {
            bindinqa.buttonSeeallReviewsDetailsMovie.visibility = View.VISIBLE
            return 10
